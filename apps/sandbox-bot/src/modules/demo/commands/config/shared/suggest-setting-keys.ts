...
export interface SettingKeyChoice {
	readonly name: string;
	readonly value: string;
}

/**
 * Suggest the declared keys whose key or translated label starts with what was
 * typed, followed by the subcommand's own extra choices.
 *
 ...
 */
export function suggestSettingKeys(
	translator: Translator,
	extraChoices: (t: Translate) => readonly SettingKeyChoice[],
	...
	return (interaction) => {
		...
		const choices: readonly SettingKeyChoice[] = [
			...DEMO_FIELD_ENTRIES.map(([key]) => ({ name: `${labelOf(key, t)} (${key})`, value: key })),
			...extraChoices(t),
		];
