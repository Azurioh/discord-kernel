...
| `color` | `"#RRGGBB"` string | Input accepts hex (`#RGB`/`#RRGGBB`, hash optional) and the base colour names of `src/color.ts`, `blurple` included (French and English, case and accents folded). Aliases a bot registers through `registerColorAliases` belong to the Discord input surface only and are rejected, so a stored value means the same colour in every bot |
